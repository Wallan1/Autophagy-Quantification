...
```{r}
testdata=read.csv(file="~/Desktop/23Jul20_MOC_Results/21Feb20_eqCellSum-MOC_basal_23Jul20.csv",header=TRUE)
testdata$BvsS<-"Basal"
testdata$Rep<-1
nobs=nrow(testdata)
for (i in 1:nobs) {
...
testdata2=read.csv(file="~/Desktop/23Jul20_MOC_Results/21Feb20_eqCellSum-MOC_starved_23Jul20.csv",header=TRUE)
...
for (t in c("WT","Mock", "Control", "tepsin", "AP4E1")) {
  for (bs in c("Basal", "Starved")) {
    temp$EnlargedPunctaCount[which(temp$Type==t & temp$BvsS==bs)]<-length(which(temp$Type==t & temp$BvsS==bs & temp$LC3Volume >2 & temp$LC3Volume < 10))
  }
}
```
