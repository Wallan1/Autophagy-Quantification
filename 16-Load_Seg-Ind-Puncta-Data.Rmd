...
*change input file path*
```{r}
testdata=read.csv(file="./Analysis-files/eqCellsum_analysis/NSW3016_EqCellSum_Basal_Vol-Int_22Feb21.csv",header=TRUE)
testdata$BvsS<-"Basal"
testdata$Rep<-1
nobs=nrow(testdata)
for (i in 1:nobs) {
...
testdata2=read.csv(file="./Analysis-files/eqCellsum_analysis/NSW3016_EqCellSum_Starved_Vol-Int_23Feb21.csv",header=TRUE)
testdata2$BvsS<-"Starved"
testdata2$Rep<-1
nobs=nrow(testdata2)
for (i in 1:nobs) {
...
for (i in 1:ntemp) {
  if (!is.na(temp[i,9]) & temp[i,9]>3750000) {
    img<-c(img,i)
  }
}
...
print list of images with high LC3 Puncta sum intensity
```{r}
...
for (i in 1:ntemp) {
  if (!is.na(temp[i,8]) & temp[i,8]>150000) {
    img<-c(img,i)
  }
}
...
for (i in 1:ntemp) {
  if (!is.na(temp[i,6]) & temp[i,6]>0.80) {
    img<-c(img,i)
  }
}
...
for (i in 1:ntemp) {
  if (!is.na(temp[i,6]) & temp[i,6]>0.5) {
    img<-c(img,i)
  }
}
