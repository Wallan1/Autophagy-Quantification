...
```{r}
testdata=read.csv(file="~/Desktop/21Jan21_analysis/final/NSW3010_Basal_count_1Feb21.csv",header=TRUE)
testdata$BvsS<-"Basal"
testdata$Rep<-1
nobs=nrow(testdata)
for (i in 1:nobs) {
...
testdata2=read.csv(file="~/Desktop/21Jan21_analysis/final/NSW3010_Starved_count_1Feb21.csv",header=TRUE)
...
*added condition to ignore NA values in LAMP counts column, must check fro presence of na values with is.na function to and manually note these erroneous images*
```{r}
ntemp<-nrow(temp)
